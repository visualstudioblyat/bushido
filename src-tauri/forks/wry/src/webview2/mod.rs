...
impl InnerWebView {
  ...
  fn new_in_hwnd(
    ...
    let controller = Self::create_controller(hwnd, &env, attributes.incognito, attributes.profile_name.as_deref(), background_color)?;
    ...
    unsafe {
      if let Ok(env10) = env10 {
        ...
        if let Some(name) = profile_name {
          ...
          controller_opts.SetProfileName(windows_core::PCWSTR::from_raw(wide.as_ptr()))?;
        }
